...
export function SecondaryCard({ project }: { project: SecondaryProject }) {
  ...
  return (
    <article className="flex h-full flex-col rounded border border-hairline bg-panel p-6 transition-colors hover:border-hairline-lit">
      ...
      <p className="mt-3 text-[15px] leading-relaxed text-text-dim">{project.note}</p>
      ...
      {link && (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="group mt-6 inline-flex items-center gap-1.5 font-mono text-[13px] text-volt transition-colors hover:text-volt-dim"
        >
          {link.label}
