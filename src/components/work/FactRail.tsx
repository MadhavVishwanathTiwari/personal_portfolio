...
function Row({ term, children }: { term: string; children: React.ReactNode }) {
  return (
    <div className="flex items-baseline justify-between gap-4 border-b border-hairline py-3 lg:flex-col lg:items-start lg:gap-1.5">
      ...
      <dd className="text-right text-[13px] text-text-dim lg:text-left">{children}</dd>
    </div>
  );
}
  ...
  return (
    <dl className="lg:sticky lg:top-24">
      ...
      {project.links.length > 0 ? (
            ...
            {project.links.map((link) => (
              <li key={link.href}>
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="group inline-flex items-center gap-1.5 font-mono text-[12px] text-volt transition-colors hover:text-volt-dim"
                >
                  {link.label}
        ...
        <p className="pt-4 font-mono text-[12px] leading-relaxed text-text-faint">
