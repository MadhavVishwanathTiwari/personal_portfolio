...
Three shots on the site, ten on the platform, including the public
certificate verifier, the resume parse step, the CSV curriculum import and
the in-app debug overlay. Sources live in `_incoming/` and the whole set is
re-runnable with `node scripts/import-kfiq-shots.mjs`, which crops browser
chrome and paints out the dev-tools bubble.
